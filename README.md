...
- Gemini images — both the ones it generates in answers and the ones you upload — are saved in full. Uploaded *files* (PDFs, docs) still come through by name only: their cards expose no real download link (the "Open" is a JS handler), so the bytes can't be pulled out.
